...
setwd("~/sce prediction_varimportance/Code")
...
#[2] Loading the Data:
# --------------------
df = read_csv("../Data/scrubbed_data_reg_final.csv") # reading the csv
colnames(df) = c('intervalTime', 'speedMean', 'speedSD', 'age', 'gender', 'prepInten',
                 'prepProb', 'windSpeed', 'visibility', 'cumDrive', 'dayOfWeek','holiday',
                 'hourDayCat', 'totalSCELag7','SCE') # setting colNames
...
recSteps = recipe(SCE ~ ., data = trainData) %>% 
  step_other(all_nominal(), - all_outcomes(), threshold = 0.05, other = "Other") %>% # group levels occuring <5% of time into Other
  step_normalize(all_numeric())  %>% # standard normal transformation for all numeric predictors
...
# [D] Model Training
models = caretList(recSteps, data = trainData, metric="ROC", 
                   tuneList= list(
                     ...
                     xgb = caretModelSpec(method="xgbTree", tuneLength=30) 
                   ),  
...
save(predResults, confMatrix, timeTaken, modelCorrs,
